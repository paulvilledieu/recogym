...
class TestAgent(Agent):
    ...
    def __init__(self, config = Configuration(test_agent_args), max_length=3):
        ...
        self.sequences = []
        self.labels = []
        self.distributions = {"n_estimators": [1, 10, 100, 200, 300, 500, 1000, 2000]}
        self.model =  skens.RandomForestClassifier()
        self.trained = False
    ...
    def act(self, observation, reward, done):
        ...
        if not self.trained:
            ...
            cv = 4
            clf = skms.GridSearchCV(self.model, self.distributions, cv=cv)
            scores = skms.cross_validate(clf, self.sequences, self.labels, return_estimator=True, cv=cv, scoring=("accuracy"))
            models = list(scores["estimator"])
            accs = list(scores["test_score"])
            self.model = scores["estimator"][np.argmax(np.array(accs))]
            self.trained = True
