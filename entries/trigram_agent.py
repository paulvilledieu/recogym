...
class TestAgent(Agent):
        ...
        if observation.sessions():
            ...
            for k in range(len(sequence) - 3):
                ...
                if not curr_item in self.bigrams:
                    self.bigrams[(curr_item, next_item, next_next_item)] = np.zeros((self.config.num_products))
                self.bigrams[(curr_item, next_item, next_next_item)][next_next_next_item] += 1
    ...
    def act(self, observation, reward, done):
        ...
        if not trigram in self.probabilities:
            action = 0
        else:
            action = self.probabilities[trigram].argmax()
